...
```bash
sam-app-with-container-image$ sam build
```
...
```bash
sam-app-with-container-image$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-app-with-container-image$ sam local start-api
sam-app-with-container-image$ curl http://localhost:3000/
```
...
```bash
sam-app-with-container-image$ sam logs -n HelloWorldFunction --stack-name sam-app-with-container-image --tail
```
...
```bash
sam-app-with-container-image$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name sam-app-with-container-image
```
